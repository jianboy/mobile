...
a)在您使用本软件网络服务，本软件自动接收并记录的您的手机上的信息，包括但不限于您的访问日期和时间、使用软件时的地理位置、主动开启的录音等数据；

2.信息的使用

a)在获得您的数据之后，本软件会将其上传至服务器，以生成问卷数据与面访问卷的录音。
...
1.1为了更好地为您提供服务，请您仔细阅读这份协议。本协议是您与本应用就您登录本应用平台进行注册及使用等所涉及的全部行为所订立的权利义务规范。

1.2为提高用户的使用感受和满意度，用户同意本应用将基于用户的操作行为对用户数据进行调查研究和分析，从而进一步优化服务。

2.服务内容

2.1智威数据是一个针对社会调研企业的信息管理平台，平台内严禁一切非法、涉黄信息，违反社区运营规范者，一律封号处理。
...
3.1.1使用本应用系统用户，只能由管理员代替其注册账号，管理员由此系统的运营者来分配。

3.1.2无法使用第三方合作网站登录。

3.2如发现用户帐号中含有不雅文字或不恰当名称的，智威数据保留取消其用户资格的权利。
...
5.3 本应用的所有内容，包括商品描述、图片等内容所有权归属于智威数据APP的用户，任何人不得转载。
